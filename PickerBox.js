...
class PickerBox extends Component {
    ...
    _renderItem = ({item, index}) => (
        <View>
            ...
            {this.state.selectedValue==item.value && <Icon type="Ionicons" name="ios-search" style={{color:'black',marginRight:10,fontSize:19}}/>}
                <Text style={[styles.text, styles.textItem, {color:this.state.selectedValue==item.value?"black":this.props.itemTextColor}]}>{item.label}</Text>
            ...
            <View style={[
                ...
                style={[styles.picker, {maxHeight: this.maxHeight}, {bottom: this.state.verticalPos}]}>
                    ...
                    <TouchableOpacity style={styles.btnVoltar} onPress={this._closePicker}>
                        <Text style={[styles.text, {color: 'gray'}]}>{ this.props.prevTextLabel }</Text>
                    </TouchableOpacity>
                </Animated.View>
            </View>
        );
